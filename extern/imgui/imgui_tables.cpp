...
void ImGui::TableUpdateLayout(ImGuiTable* table)
{
    ...
    const float width_avail = ImMax(1.0f, (((table->Flags & ImGuiTableFlags_ScrollX) && table->InnerWidth == 0.0f) ? table->InnerClipRect.get_width() : work_rect.get_width()) - width_removed);
...
void    ImGui::EndTable()
{
    ...
    table_instance->LastOuterHeight = table->OuterRect.get_height();
...
void ImGui::DebugNodeTable(ImGuiTable* table)
{
    ...
    BulletText("OuterRect: Pos: (%.1f,%.1f) Size: (%.1f,%.1f) Sizing: '%s'", table->OuterRect.Min.x, table->OuterRect.Min.y, table->OuterRect.get_width(), table->OuterRect.get_height(), DebugNodeTableGetSizingPolicyDesc(table->Flags));
